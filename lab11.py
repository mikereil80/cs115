...
def insertV1(L, i):
    ...
    sortedlist=L[0:i]
    unsortedvalue=L[i]
    for index in reversed(range(0,i)):
        if unsortedvalue<sortedlist[index]:
...
def search(L, i, x):
    ...
    if L[0]>x:
        ...
        for index in range(0,i):
            if sortedlist[index-1]<=x and sortedlist[index]>x:
                j=index
    return j
    ...
    for l in line:
        for char in l:
            ...
            if lowercase in alphabet:
                alphabet[lowercase]+=1
            else:
                alphabet[lowercase]=1
    keys=alphabet.keys()
    keysort=reversed(sorted(keys))
    for index in keysort:
        finallist+=[(alphabet[index], index)]
    sortlist=sorted(finallist, reverse=True)
    return sortlist
