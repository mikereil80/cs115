...
def gor3(x,y,z):
    '''Or of three inputs.'''
    return gand(gor(gor(x,y),gor(y,z)),gor(gor(x,z),gor(x,y)))
...
def FA(x,y,cin):
    ...
    s=x+y+cin
    carry_out=gor(x,y)+gor(x,cin)+gor(y,cin)
    return (carry_out%2, s%2)
...
def fourBitAdd(xxxx,yyyy):
    ...
    (c,zf)=FA(xxxx[1], yyyy[1], b)
    (d,ze)=FA(xxxx[0], yyyy[0], c)
    return (d, (ze,zf,zt,zo))
...
def test_fourBitAdd():
    ...
    c,ww = fourBitAdd(fifteen, fifteen)
